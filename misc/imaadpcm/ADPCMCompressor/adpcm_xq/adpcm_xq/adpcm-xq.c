...
int main (int argc, char *argv[])
{
    ...
    while (--argc) {
        ...
        if ((**++argv == '-') && (*argv)[1])
            ...
            infilename = (char*)malloc (strlen (*argv) + 10);
            strcpy (infilename, *argv);
        }
        else if (!outfilename) {
            outfilename = (char*)malloc (strlen (*argv) + 10);
...
static int adpcm_decode_data (FILE *infile, FILE *outfile, int num_channels, size_t num_samples, int block_size)
{
    ...
    while (num_samples) {
		...
		if (adpcm_decode_block ((int16_t*)pcm_block, (const uint8_t*)adpcm_block, block_size, num_channels) != this_block_adpcm_samples) {
...
static int adpcm_encode_data (FILE *infile, FILE *outfile, int num_channels, size_t num_samples, int samples_per_block, int lookahead, int noise_shaping)
{
    ...
    while (num_samples) {
        ...
        adpcm_encode_block (adpcm_cnxt, (uint8_t*)adpcm_block, &num_bytes, pcm_block, this_block_adpcm_samples);
