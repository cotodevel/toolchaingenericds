...
uint32_t get_file_date(const char *path, uint32_t *dos_date)
{
    ...
    if (strcmp(path, "-") != 0)
    {
        ...
        char *name = (char *)TGDSARM9Malloc(len + 1);
...
int makedir(const char *newdir)
{
    ...
    buffer = (char*)TGDSARM9Malloc(len + 1);
    if (buffer == NULL)
    {
...
FILE *get_file_handle(const char *path)
{
    ...
    pathWide = (wchar_t*)TGDSARM9Calloc(pathLength, sizeof(wchar_t));
    MultiByteToWideChar(CP_UTF8, 0, path, -1, pathWide, pathLength);
    handle = _wfopen((const wchar_t*)pathWide, L"r");
    TGDSARM9Free(pathWide);
#else
    handle = fopen(path, "r");
#endif
